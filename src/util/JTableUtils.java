...
public class JTableUtils {
    ...
    /**
     * Запись данных из списка (одномерного или двумерного) в JTable
     * (основная реализация, закрытый метод).
     */
    private static void writeListToJTable(JTable table, List<?> list, String itemFormat) {
        ...
        boolean is2D = list.get(0) instanceof List<?>;
        int rowCount = is2D ? list.size() : 1;
        int colCount = is2D
                ? list.stream().mapToInt(row -> ((List<?>) row).size()).max().orElse(0)
                : list.size();
        ...
        if (is2D) {
            for (int i = 0; i < list.size(); i++) {
                ...
                for (int j = 0; j < row.size(); j++) {
                    tableModel.setValueAt(String.format(itemFormat, row.get(j)), i, j);
                }
            }
        } else {
            for (int i = 0; i < list.size(); i++) {
                tableModel.setValueAt(String.format(itemFormat, list.get(i)), 0, i);
            }
        }
    ...
    /**
     * Запись данных из списка List<Integer> в JTable
     * (основная реализация, закрытый метод)
     */
    public static void writeListToJTable(JTable table, List<Integer> list) {
        writeListToJTable(table, list, "%d");
    }

    /**
     * Запись данных из списка List<List<Integer>> в JTable
     * (основная реализация, закрытый метод)
     */
    public static void writeList2ToJTable(JTable table, List<List<Integer>> list) {
        writeListToJTable(table, list, "%d");
    }
    ...
    /**
     * Чтение данных из JTable в двухмерный список
     *
     * @param table           JTable, из которого считываются данные
     * @param clazz           Класс типа элементов
     * @param converter       Функция для преобразования строк в нужный тип
     * @param errorIfEmptyCell Флаг, выбрасывать ли исключение при пустой ячейке
     * @param emptyCellValue  Значение по умолчанию для пустых ячеек
     * @param ignoreEmptyCell Игнорируем ли пустые ячейки
     * @param <T>             Тип элементов
     * @return Двухмерный список, содержащий данные из JTable
     * @throws JTableUtilsException Если пустая ячейка и установлен флаг errorIfEmptyCell
     */
    public static <T> List<List<T>> readList2FromJTable(
            JTable table, Class<T> clazz, Function<String, ? extends T> converter,
            boolean errorIfEmptyCell, T emptyCellValue, boolean ignoreEmptyCell
    ) throws JTableUtilsException {
        TableModel tableModel = table.getModel();
    ...
    /**
     * Чтение данных из JTable в одномерный список
     */
    public static <T> List<T> readListFromJTable(
            JTable table, Class<T> clazz, Function<String, ? extends T> converter,
            boolean errorIfEmptyCell, T emptyCellValue, boolean ignoredEmptyCell
    ) throws JTableUtilsException {
        List<List<T>> matrix = readList2FromJTable(table, clazz, converter, errorIfEmptyCell, emptyCellValue, ignoredEmptyCell);
        return matrix.get(0);
    }
    ...
    /**
     * Чтение данных из JTable в одномерный массив int[]
     */
    ...
    /**
     * Чтение данных из JTable в одномерный список List<Integer>
     */
    public static List<Integer> readIntListFromJTable(JTable table) throws ParseException {
        try {
            return readListFromJTable(table, Integer.class, Integer::parseInt, false, null, true);
        } catch (JTableUtilsException ignored) {
        }
        return null;
    }
    ...
    /**
     * Чтение данных из JTable в двумерный список List<List<Integer>>
     */
    public static List<List<Integer>> readIntList2FromJTable(JTable table) throws ParseException {
        try {
            return readList2FromJTable(table, Integer.class, Integer::parseInt, false, null, true);
        } catch (JTableUtilsException ignored) {
        }
        return null;
    }
